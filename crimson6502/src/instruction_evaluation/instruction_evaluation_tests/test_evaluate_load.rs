...
pub fn test_load_zeropage_helper(mnemonic: Mnemonic, expected_value: u8, addr: u8, addressing_mode: AddressingMode, index_value: u8) {
    ...
    match addressing_mode {
        AddressingMode::ZeroPageX => cpu_state.x = index_value,
        AddressingMode::ZeroPageY => cpu_state.y = index_value,
        _ => (),
    };
    ...
    let effective_addr: u8 = match addressing_mode {
        ...
        AddressingMode::ZeroPageX => addr.wrapping_add(index_value),
        AddressingMode::ZeroPageY => addr.wrapping_add(index_value),
        _ => panic!("what the fuck is this addressing mode? {:?}", addressing_mode),
    };
...
pub fn test_load_absolute_helper(mnemonic: Mnemonic, expected_value: u8, addr: u16, addressing_mode: AddressingMode, index_value: u8) {
    let mut cpu_state = CPUState::new();
    match addressing_mode {
        AddressingMode::AbsoluteX => cpu_state.x = index_value,
        AddressingMode::AbsoluteY => cpu_state.y = index_value,
        _ => (),
    };
    ...
    let effective_addr: u16 = match addressing_mode {
        ...
        AddressingMode::AbsoluteX => addr.wrapping_add(index_value as u16),
        AddressingMode::AbsoluteY => addr.wrapping_add(index_value as u16),
        _ => panic!("what the fuck is this addressing mode? {:?}", addressing_mode),
    };
